...
local function makeDraggable(dragHandle, target)
    ...
    local function updatePos(input)
        ...
        local halfW = target.AbsoluteSize.X * 0.5
        local halfH = target.AbsoluteSize.Y * 0.5
        local desiredX = startPos.X.Offset + delta.X + startPos.X.Scale * viewport.X
        local desiredY = startPos.Y.Offset + delta.Y + startPos.Y.Scale * viewport.Y
        local minX = halfW + 8
        local maxX = viewport.X - halfW - 8
        local minY = halfH + 8
        local maxY = viewport.Y - halfH - 8
        if maxX < minX then maxX = minX end
        if maxY < minY then maxY = minY end
        target.Position = UDim2.new(0, math.clamp(desiredX, minX, maxX), 0, math.clamp(desiredY, minY, maxY))
    end
...
Aurora.WindowMinSize = Vector2.new(300, 240)
...
function Aurora:CreateWindow(config)
    ...
    local function computeFittedSize()
        local viewport = workspace.CurrentCamera and workspace.CurrentCamera.ViewportSize or Vector2.new(1280, 720)
        local maxW = math.max(Aurora.WindowMinSize.X, viewport.X - 32)
        local maxH = math.max(Aurora.WindowMinSize.Y, viewport.Y - 32)
        local w = math.clamp(config.Width or WINDOW_WIDTH, Aurora.WindowMinSize.X, math.min(Aurora.WindowMaxSize.X, maxW))
        local h = math.clamp(config.Height or WINDOW_HEIGHT, Aurora.WindowMinSize.Y, math.min(Aurora.WindowMaxSize.Y, maxH))
        return w, h, viewport
    end

    local windowWidth, windowHeight = computeFittedSize()
    ...
    local function clampToViewport()
        ...
        if maxX < minX then maxX = minX end
        if maxY < minY then maxY = minY end
        local currentX = main.Position.X.Offset + main.Position.X.Scale * viewport.X
        local currentY = main.Position.Y.Offset + main.Position.Y.Scale * viewport.Y
    ...
    local function refitToViewport()
        local w, h = computeFittedSize()
        windowWidth, windowHeight = w, h
        if math.abs(main.Size.Y.Offset - 44) < 1 then
            main.Size = UDim2.new(0, w, 0, 44)
        else
    ...
    if workspace.CurrentCamera then
        workspace.CurrentCamera:GetPropertyChangedSignal("ViewportSize"):Connect(refitToViewport)
    end
    ...
    closeBtn.MouseButton1Click:Connect(function()
        ...
        if tabRail then tween(tabRail, {BackgroundTransparency = 1}, 0.12) end
        if pageHolder then
    ...
    minimizeBtn.MouseButton1Click:Connect(function()
        ...
        if minimized then
            ...
            if tabRail then tween(tabRail, {BackgroundTransparency = 1}, 0.14) end
            task.wait(0.1)
            ...
            task.wait(0.18)
            if tabRail then tween(tabRail, {BackgroundTransparency = 0.3}, 0.2) end
            if pageHolder then
                for _, d in ipairs(pageHolder:GetDescendants()) do
                    if d:IsA("GuiObject") then
...
local function renderControls(theme, container, boxed)
    ...
    function api.Frame3D(cfg)
        ...
        local FRAME_THEMES = {
            Grey = Color3.fromRGB(140, 140, 148),
            Accent = theme.Accent,
        ...
        corner(holder, 10)
        stroke(holder, theme.Border, 1).Transparency = 0.35
        ...
        local viewport = new("ViewportFrame", {
            ...
            LightDirection = Vector3.new(-0.4, -1, -0.6).Unit,
        ...
        part.Size = cfg.PartSize or Vector3.new(2, 2, 2)
        ...
        local radius = math.max(part.Size.X, part.Size.Y, part.Size.Z) * 2.2
        local function frame(t)
            local angle = t * (cfg.Speed or 0.6)
            local camPos = Vector3.new(math.sin(angle) * radius, radius * 0.55, math.cos(angle) * radius)
            camera.CFrame = CFrame.new(camPos, Vector3.new(0, 0, 0))
            part.CFrame = CFrame.new(0, 0, 0) * CFrame.Angles(angle * 0.6, angle, angle * 0.3)
        end
        ...
        return {
            ...
            SetPart = function(_, instance)
                if instance and instance:IsA("BasePart") then
                    part:Destroy()
                    instance.Parent = worldModel
                    instance.Anchored = true
                    instance.CanCollide = false
                    part = instance
                    radius = math.max(part.Size.X, part.Size.Y, part.Size.Z) * 2.2
                end
            end,
            Viewport = viewport,
